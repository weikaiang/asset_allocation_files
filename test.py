...
def create_dataloader(config, device):
    ...
    # 定义训练器的的输入
    true_inout_seq = create_inout_sequences(true_data_normalized, train_window, pre_len)

    # 定义因子训练器的的输入
    true_fac_inout_seq = create_inout_sequences(true_data_fac_normalized, train_window, pre_len)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        ...
        values = values.reshape(num, value_len, self.heads, self.head_dim)
        keys = keys.reshape(num, key_len, self.heads, self.head_dim)
...
def predict(model, args, device, scaler, data, rolling_data=None):
    # 预测未知数据的功能
    # 重新读取数据
...
def rolling_predict(model, args, device, scaler, data):
    # 滚动预测
    pre_data = data.copy()
    columns = pre_data.columns[0:]
    ...
    for i in range(int(len(pre_data) / args.pre_len)):
        ...
        pred = predict(model, args, device, scaler, pre_data, rolling_data)
        pred_array = pred.values #将dataframe转化为numpy数组
        for j in range(args.output_size):
            for k in range(args.pre_len):
                temp = pred_array[k][j]
                dict_of_lists[columns[j]].append(temp)
...
def LSTMargs(data_df, window_size=60, pre_len=15, size=8, lr=0.001, drop_out=0.05, epochs=50, batch_size=16):
    """
...
def pred(args):
    ...
    #    pred = rolling_predict(model, args, device, scaler_train, rate_df)
    return pred

if __name__ == '__main__':
    data = pd.read_csv('asset.csv')
    args = LSTMargs(data, epochs = 10)
    pred_val = pred(args)
    print(pred_val)